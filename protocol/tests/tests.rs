...
quickcheck! {
    fn test_punsh(address: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<Punsh>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
...
quickcheck! {
    fn test_get_block(block: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<GetBlock>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
...
quickcheck! {
    fn test_get_block_ack(filename: String, index: u64, timestamp: i64, nonce: u64, prev: String, hash: String, content: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<GetBlockAck>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
...
quickcheck! {
    fn test_block_data(unique_key: String, content: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<BlockData>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
...
quickcheck! {
    fn test_block_gen(index: u64, timestamp: i64, prev: String, sign_key: String, content: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<BlockGen>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
        assert_eq!(index, blockchain_parsed.payload.index);
        assert_eq!(content, blockchain_parsed.payload.content);
        assert_eq!(timestamp, blockchain_parsed.payload.timestamp);
...
quickcheck! {
    fn test_block_found(index: u64, timestamp: i64, nonce: u64, hash: String, prev: String, content: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<BlockFound>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
        assert_eq!(index, blockchain_parsed.payload.index);
        assert_eq!(timestamp, blockchain_parsed.payload.timestamp);
        assert_eq!(nonce, blockchain_parsed.payload.nonce);
        assert_eq!(prev, blockchain_parsed.payload.prev);
...
quickcheck! {
    fn test_hash_val(index: u64, timestamp: i64, nonce: u64, prev: String, content: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<HashVal>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
        assert_eq!(index, blockchain_parsed.payload.index);
        assert_eq!(content, blockchain_parsed.payload.content);
        assert_eq!(timestamp, blockchain_parsed.payload.timestamp);
        assert_eq!(nonce, blockchain_parsed.payload.nonce);
...
quickcheck! {
    fn test_hash_val_ack(index: u64, hash: String) -> bool {
        ...
        let blockchain_parsed = Protocol::<HashValAck>::from_bytes(&blockchain_protocol, &there_nacl, &our_nacl.get_public_key()).unwrap();
        assert_eq!(index, blockchain_parsed.payload.index);
        assert_eq!(hash, blockchain_parsed.payload.hash);
        true
    }
