...
fn test_basic() {
    ...
    let expected = vec![255, 255, 0, 0, 0, 0, 0, 0, 209, 95, 147, 191];
...
fn test_set_payload() {
    ...
    let expected = vec![255, 255, 0, 0, 0, 0, 0, 0, 209, 95, 147, 191, 126, 77, 121, 32, 116, 101, 115, 116, 32, 112, 97, 121, 108, 111, 97, 100, 126];
    assert_eq!(blockchain_protocol.build(), expected);
}
...
fn test_set_all_values() {
    ...
    let expected = vec![16, 0, 0, 0, 0, 0, 0, 0, 25, 153, 238, 66, 126, 49, 57, 50, 46, 49, 54, 56, 46, 49, 57, 50, 46, 50, 58, 52, 53, 54, 55, 56, 126];
    assert_eq!(blockchain_protocol.build(), expected);
}