...
/// Model for the event `NewBlockContent`
///
/// ```
...
impl Payload for NewBlockContent {
    fn new() -> Self {
...
mod tests {
    ...
    fn test_building_and_parsing() {
        ...
        let data = NewBlockContent {
            unique_key: unique_key.clone(),
            content: content.clone()
        };

        let complete = Parser::parse_payload(&data.to_bytes());
        let parsed = NewBlockContent::parse(complete).unwrap();
    ...
    quickcheck! {
        ...
        fn test_quickcheck(content: String) -> bool {
            ...
            let mut data = NewBlockContent::new();
